...
const WithdrawButton = ({ enable }) => {
  ...
  function withdraw() {
    if (!processing) {
      ...
      callAction(market, methodName, [TOKENS_CONTRACT_ADDRESS])
        .then(() => {
          setProcessing(false);
        })
        .catch((err) => {
...
const CollectFees = () => {
  ...
  return (
    <div>
      <Separator />
      <Row>
        {`Creator Fees: ${utils.formatEther(fee)} ${TOKEN_NAME}`}{" "}
        <WithdrawButton enable={fee > 0} />
      </Row>
    </div>
  );
};
...
export function AdminPanel() {
  const { market, isInMarket, creatorFee, minPrice, isMarketAdmin } =
    useContract();
  ...
  return (
    <div style={listStyle}>
      ...
      <DetailWrapper>
        ...
        <div>
          <ButtonGroup>
            <Btn className="btn" disabled={processing} onClick={save}>
              {isInMarket ? "Save Settings" : "Setup Market"}
            </Btn>
          </ButtonGroup>
        </div>
      </DetailWrapper>
