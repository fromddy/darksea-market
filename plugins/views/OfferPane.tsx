    ...
    .map((offer) => {
      ...
      const rows = [
        <tr key={offer.offerId.toString()} style={centerTable}>
          <td>
            <ArtifactRarityTypeLabelAnim artifact={offer} isOffer={true} />
          </td>
          <td>
            <ArtifactRarityLabelAnim artifact={offer} />
          </td>
          <td>{offer.qty.toString()}</td>
          <td>{offer.deal.toString()}</td>
          <td>
            <PriceCell>{utils.formatEther(offer.price)}</PriceCell>
          </td>
          <td>
      ...
      if (act) {
        rows.push(
          <tr key={offer.offerId.toString() + "a"}>
            <td colSpan="6">
    ...
    <div style={listStyle}>
      ...
      {loading ? (
        ...
        <table style={table}>
          ...
          />
          <tbody>{offerChildren}</tbody>
        </table>
      ) : (
        <div style={textCenter}>No offer right now.</div>
      )}
      {mine ? <CreateOffer /> : ""}
    </div>
  );
