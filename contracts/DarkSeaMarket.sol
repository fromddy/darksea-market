...
contract DarkSeaMarket is Initializable, DarkSeaMarketStorage {
    ...
    modifier isApproved(address token, uint256 tokenID) {
        ...
        require(
            t.getApproved(tokenID) == address(this) ||
                t.isApprovedForAll(t.ownerOf(tokenID), address(this)),
            "Approve require"
        );
        _;
    }
    ...
    modifier onlyCollectionOwner(address token) {
        require(
            s.collections[token].owner == msg.sender,
            "Not the owner of collection"
        );
        _;
    }
    ...
    function list(
        ...
        uint256 tokenID,
        uint256 price
    ) external tokenExists(token) notPaused(token) isApproved(token, tokenID) {
    ...
    function buy(
        address token,
        uint256 listId
    ) public payable tokenExists(token) notPaused(token) {
        DarkSeaMarketTypes.Item memory item = s.collections[token].listings[
            listId
        ];
    ...
    function getItem(
        ...
        view
        tokenExists(token)
        returns (DarkSeaMarketTypes.Item memory item)
    {
        item = DarkSeaMarketList.getItem(token, listId);
    }

    function bulkGetItems(
        address token,
        uint256 startIdx,
        uint256 endIdx
    )
        ...
        view
        tokenExists(token)
        returns (DarkSeaMarketTypes.Item[] memory ret)
    {
    ...
    function getNItemsByOwner(
        address token,
        address owner
    ) public view tokenExists(token) returns (uint256) {
    ...
    function getMyItems(
        ...
        view
        tokenExists(token)
        returns (DarkSeaMarketTypes.Item[] memory)
    {
    ...
    ) external payable tokenExists(token) notPaused(token) {
        require(msg.value == price * qty, "Price too low");
        uint256 offerId = DarkSeaMarketOffer.placeOffer(
            token,
            price,
            qty,
            rarity,
    ...
    function fillOffer(
        ...
        uint256 offerId,
        uint256 tokenID
    ) external tokenExists(token) isApproved(token, tokenID) {
    ...
    function getOffer(
        ...
        view
        tokenExists(token)
        returns (DarkSeaMarketTypes.Offer memory ret)
    {
        ret = DarkSeaMarketOffer.getOffer(token, offerId);
    }

    function bulkGetOffers(
        address token,
        uint256 startIdx,
        uint256 endIdx
    )
    ...
    function getAllOffers(
        ...
        view
        tokenExists(token)
        returns (DarkSeaMarketTypes.Offer[] memory)
    {
    ...
    function getCollectionFee(
        address token
    ) public view tokenExists(token) returns (uint256 fee) {
        fee = s.collections[token].fee;
    }

    function getCollectionMinPrice(
        address token
    ) public view tokenExists(token) returns (uint256) {
    ...
    function editByOwner(
        address token,
        uint256 fee,
        uint256 minPrice
    ) external tokenExists(token) onlyCollectionOwner(token) {
        require(minPrice >= 0.01 ether, "Min Price must greater than 0.01");
        _editCollection(token, msg.sender, fee, minPrice);
    }

    function collectCollectionFees(
        address token
    ) external tokenExists(token) onlyCollectionOwner(token) {
    ...
    function transferCollectionOwner(
        address token,
        address newOwner
    ) external tokenExists(token) onlyCollectionOwner(token) {
    ...
    function getCollectionFeeBalance(
        address token
    )
        external
        view
        tokenExists(token)
        onlyCollectionOwner(token)
        returns (uint256)
    {
        return s.collections[token].feeBalance;
    }

    function getCollectionTotalFee(
        address token
    )
        external
        view
    ...
    function addCollection(
        ...
        uint256 fee,
        uint256 minPrice
    ) external onlyOwner tokenNotExists(token) {
    ...
    function editCollection(
        ...
        uint256 fee,
        uint256 minPrice
    ) external onlyOwner tokenExists(token) {
